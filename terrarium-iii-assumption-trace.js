...
function codeAudit(code){
  ...
  return {world:[...new Set(world)],ticks,solids,surfaces,groups,namedStructure,directPut};
}
function referenceRows(c){return rows(
...
function readRows(c){
  ...
  return rows(
    ...
    row('system','INVARIANTS',arr(s.invariants).map(x=>typeof x==='string'?x:json(x,220)).join(' · '),'locked after Reader; repair should not move these goalposts'),
    row('system','FORBIDDEN TRANSLATIONS',arr(s.forbidden_translations).map(x=>typeof x==='string'?x:json(x,180)).join(' · '),'explicit anti-drift constraints')
  )
}
function planRows(c){
  const p=c?.plan||{};
...
function buildRows(c){
  const p=c?.plan||{},last=arr(c?.passes).at(-1),audit=last?.code?codeAudit(last.code):null;
  const out=rows(
...
function observedRows(c,pass){
  const p=arr(c?.passes).find(x=>x.pass===pass)||arr(c?.passes).at(-1),m=p?.metrics||{},audit=p?.code?codeAudit(p.code):null;
  return rows(
    ...
    audit&&!audit.namedStructure?row('warning','STRUCTURE ROOT','No explicit named STRUCTURE root detected.','source contradicts bounded-building contract'):null,
    audit&&audit.ticks?row('warning','ANIMATION',`${audit.ticks} WG.tick call(s)`,'unexpected motion inside static structure build'):null
  )
}
...
function ensurePanel(){
  ...
  if(!panel){panel=document.createElement('section');panel.id='iii-assumption-trace';panel.style.cssText='order:0;margin:8px 0;border:1px solid #303942;background:#070a0d;padding:8px;font:10px/1.42 monospace;color:#e9eef2';const note=flow.querySelector('#iii-ref-note');if(note)flow.insertBefore(panel,note);else flow.appendChild(panel)}
  return panel
}
...
function render(active){
  ...
  panel.innerHTML=`<div style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:6px"><b style="letter-spacing:.18em;color:#19e6c8">ASSUMPTION TRACE</b><span style="color:#7f8a94">${esc(VERSION)}</span></div>`+shown.map(h=>{const isActive=h.seq===active.seq;const body=h.rows.map(r=>`<div style="border-top:1px solid #1c232a;padding:5px 0"><div><b style="color:${color(r.kind)}">${esc(r.kind.toUpperCase())}</b> · <b>${esc(r.name)}</b></div><div style="color:#d9dfe4;margin-top:2px">${esc(r.value)}</div>${r.basis?`<div style="color:#75808a;margin-top:2px">basis · ${esc(r.basis)}</div>`:''}</div>`).join('');return `<details ${isActive?'open':''} style="border-top:1px solid ${isActive?'#19e6c8':'#252d34'};padding:5px 0"><summary style="cursor:pointer;color:${isActive?'#19e6c8':'#9aa5ae'}"><b>STEP ${esc(h.stage)}</b>${h.pass?` · PASS ${h.pass}`:''}</summary>${body||'<div style="color:#75808a;padding:5px 0">waiting for assumptions…</div>'}</details>`}).join('')
}
async function connection(){
  try{const cfg=global.III_NET?.config?.()||{},url=String(cfg.url||'').replace(/\/+$/,''),key=String(cfg.key||''),create=global.supabase?.createClient;if(typeof create!=='function'||!url||key.length<20)return null;const sig=url+'|'+key;if(!sb||sbSig!==sig){sbSig=sig;sb=create(url,key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}})}const r=await sb.auth.getSession();if(r.error||!r.data?.session?.user?.id)return null;return sb}catch(_){return null}
}
async function persist(p){
  if(!p.buildId)return;const key=`${p.buildId}|${p.stage}|${p.pass}|${JSON.stringify(p.rows)}`;if(persisted.has(key))return;persisted.add(key);const client=await connection();if(!client)return;const detail=p.rows.map(r=>`${r.kind}:${r.name}=${r.value}`).join(' | ').slice(0,4000);const r=await client.from('iii_trace_events').insert({build_id:p.buildId,phase:'assumption',label:`ASSUMPTIONS · ${p.stage}${p.pass?` · PASS ${p.pass}`:''}`,detail,payload_json:p}).select('id').maybeSingle();if(r.error)console.warn('[III ASSUMPTIONS persist]',r.error)
}
function tick(){
  ...
  const title=currentTitle(),s=stage(title,c),p=payload(s,c,title),signature=JSON.stringify({stage:p.stage,pass:p.pass,rows:p.rows}),last=history.at(-1);
  if(!last||last.signature!==signature){const h={...p,seq:++seq,signature};if(last&&last.stage===h.stage&&last.pass===h.pass)history[history.length-1]=h;else history.push(h);if(history.length>18)history.shift();render(h);persist(p).catch(()=>{});global.__iiiAssumptionTrace=history.map(x=>({...x,signature:undefined}));try{document.dispatchEvent(new CustomEvent('iii-architecture-assumptions',{detail:p}))}catch(_){}}
  else render(last)
}

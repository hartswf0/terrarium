...
// ============================================================================
const IN=0.0254;                                   // the trailer is authored in inches
...
// STRUCTURE — transcribed from gunnars-depot operative/ingold.js (SHELL/openings/
// interior, abridged): plan inches, x across width 0..101, y along length 0..240,
// z up from datum. door.entry: wall W, 72..108, per the plan. Wall pieces are
// SPLIT around real openings, so a door is an actual hole and a window is glass.
const WT=4.5, DH=DECK+80*IN;                       // wall thickness, door head
...
const DOOR={id:'door.entry',wall:'W',from:72,to:108,x:px2w(0),z0:py2w(72),z1:py2w(108)};
...
function updateWorldUI(){
  ...
  const d=Math.hypot(ball.p.x-locomotion.root.x,ball.p.z-locomotion.root.z);
  bb.textContent=ball.state==='hero'?'THROW':ball.state==='dog'?'ARGOS…':(d<=1.05?'TAKE':'BALL');

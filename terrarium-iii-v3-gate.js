...
function reassertV3(engine){
  ...
  // A later registration replaced the V3 wrapper. Put only the V3 request back
  // onto the current domain so its current compile/apply/anchor remain intact.
...
function install(){
  ...
  if(typeof v3Request!=='function')return false; // wait until V3 has wrapped draft once
  ...
  const gated=async function iiiV3SharedSay(domainId,text,imageDataUrl){
    ...
    if(!result?.ok){
      // runAIAgent treats ok:false as permission to fall through to golden and
      // legacy arena generation. For a house this is forbidden: consume the
      // request as a visible stopped V3 operation instead.
      blockedFallbacks++;
      clearStaleDraft();
...
function arm(){
  if(install())return;
  let n=0;
  timer=setInterval(()=>{n++;if(install()||n>240){clearInterval(timer);timer=0}},250);
}
arm();
